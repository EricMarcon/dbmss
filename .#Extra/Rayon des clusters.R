...
# Cluster de Matérn bien serrés, fenêtre carré de 1x1. Le rayon des clusters est 0.05
X <- rMatClust(20, 0.05, 10)
X <- as.wmppp(X)
plot(X)
# r entre 0 et 0.1. Le 0 initial est obligatoire pour spatstat.
rseq <- seq(0, 0.1, 0.005)

(K <- Khat(X, r=rseq))
# J'aime bien la normalisation par pi.r² mais ça ne vaut rien pour évaluer le rayon
plot(K, ./(pi*r^2) ~ r)

# Calcul de la variance exacte, grâce à notre code de Ktest (ci-dessous, exécuter tout ce qui est sous "Utilities" en premier)
# Les valeurs de r sont sans le 0 initial: rseq[-1]
# C'est long, la matrice est 20x20
Variance <- sigmaKi(rseq[-1], X$n, 1, 1)
# Calcul de Ktilde (eq. 4)
Ktilde <- (K$iso[-1]-pi*rseq[-1]*rseq[-1])/sqrt(diag(Variance))
# Figure. On espère avoir le pic à .05*1.3=0.065
plot(x=rseq[-1], y=Ktilde)
# Comparer avec L
(L <- Lhat(X, r=rseq))
plot(L)
...
# Utilities : à exécuter en premier, collé à la fin pour la lisibilité.
# r : radius
