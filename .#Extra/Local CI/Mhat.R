Mhat <- function(
  ...
  if (!is.null(ReferencePoint)) {
    # Set individual to TRUE if a reference point is given
    Individual <- TRUE
  ...
  if (ReferenceType == NeighborType | CaseControl) {
    WrMinusReferencePoint <- sum(marks(X)$PointWeight[IsReferenceType]) -
      marks(X)$PointWeight
  ...
  if (is.null(ReferencePoint)) {
    ...
    Mvalues <- apply(
      LocalRatio,
      MARGIN = 2,
      FUN = function(x) sum(x[is.finite(x)]) / sum(GlobalRatio[is.finite(x)])
    )
    ...
    ReferencePoint_index <- which(
      rownames(marks(X[IsReferenceType])) == ReferencePoint_name
    )
